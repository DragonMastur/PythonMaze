...
class GameScreen:
	...
	def c_square(self, x1, y1, x2, y2, depth, fill='white', outer='black'):
		...
		# calculate the z1 and z2 positions
		z1 = depth
		z2 = depth
...
class Game:
	...
	def update(self):
		self.GS.update()
		if self.dem == 2:
			if self.GS.gameboard[self.position[0]][self.position[1]].outer != 'none':
				self.GS.c_rect(self.position[0]*self.GS.rectw, self.position[1]*self.GS.recth, (self.position[0]+1)*self.GS.rectw, (self.position[1]+1)*self.GS.recth, self.poscolor, self.GS.gameboard[self.position[0]][self.position[1]].outer, self.GS.gameboard[self.position[0]][self.position[1]].text)
			else:
				self.GS.c_rect(self.position[0]*self.GS.rectw, self.position[1]*self.GS.recth, (self.position[0]+1)*self.GS.rectw, (self.position[1]+1)*self.GS.recth, self.poscolor, text=self.GS.gameboard[self.position[0]][self.position[1]].text)
		else:
			raise VersionError("3D mapping not avalable yet. Sorry")
		try:
			self.onUpdateExc()
		except:
			pass
	...
	def checkPos(self):
		...
		if self.position[0]>int(self.GS.rootgeo[0]/self.GS.rectw)-1: self.position[0]=int(self.GS.rootgeo[0]/self.GS.rectw)-1;
		if self.position[1]>int(self.GS.rootgeo[1]/self.GS.recth)-1: self.position[1]=int(self.GS.rootgeo[1]/self.GS.recth)-1;
